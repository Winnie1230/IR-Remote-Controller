...
//---------函數放的地方-----------------
 ...
 //-----------------------------------
 void namewrite2(uint16_t pos , String Name) {      //把name寫入eeprom的function
      uint8_t Length = Name.length();
      for (uint8_t i = 0 ; i < Length ; i++) {
        eeprommatrix[pos] = Name[i];
        pos++;
 ...
 void nameread2(uint16_t pos, String& Return) {      //讀出eeprom中的name
    uint8_t Length =  eeprommatrix[pos]; //第一位記錄Name的長度
    char Name[8];
    for (int i = 0; i < 8 ; i++) {
      Name[i] =  eeprommatrix[pos];
...
//-------btnremove---------
void btnremove(String button){
  uint16_t startaddr = ircodestart;
  String goal;
  uint16_t irlength;
  uint16_t endpos = startaddr;
  
  do{
    startaddr = endpos;
    endpos += nameread(startaddr, goal);
    numberread(endpos,irlength);
    endpos +=2;
    endpos += irlength*2;
  }while( button != goal);
   for(int i = 0 ; i<(currentPosition-endpos) ;i++){
      eeprommatrix[startaddr+i] = eeprommatrix[endpos+i];
   }
   currentPosition = currentPosition - (endpos - startaddr);
