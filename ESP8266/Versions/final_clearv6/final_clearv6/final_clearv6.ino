...
void callback(char* topic, byte * payload, unsigned int length) {
  ...
  if ( strcmp(topic, "qwe/APP/start") == 0){ //APP開啟 Publish所有按鍵名稱
    eepromload();
    publishButtonName();
  }
  else if (strcmp(topic, "qwe/APP/learn") == 0){   //APP學習按鍵
    eepromload();
    learnft();
    eeprompush();
  }
  else if (strcmp(topic, "qwe/APP/control") == 0){ //發射紅外碼
    eepromload();
    emit();
  }
  else if (strcmp(topic, "qwe/Web") == 0){        //傳送sensor 開關狀態給server
