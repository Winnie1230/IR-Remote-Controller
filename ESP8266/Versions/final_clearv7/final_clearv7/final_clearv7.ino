...
void callback(char* topic, byte * payload, unsigned int length) {
  ...
  if ( strcmp(topic, "/APP/start")== 0){ //APP開啟 Publish所有按鍵名稱
    ...
    String state;
    nameread2(2,state);
    CO2state = state[0]; //二氧化碳感測器
    pm25state = state[1]; //pm2.5狀態
    pm10state = state[2]; //pm10狀態
    hchostate = state[3]; //hcho狀態
    tvocstate = state[4]; //TVOC狀態
    humidstate = state[5]; //濕度狀態
    temperstate = state[6]; //溫度狀態
